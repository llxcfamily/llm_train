...
def preprocess(
    ...
    for model_input in model_inputs:
        # add bos token in the start of seq
        tokenized_text = [1]
        labeled = [IGNORE_INDEX]
        # multi-turn dialogue processing
        turns = model_input.split(seperators[1])
        #print(len(turns))
        for i, turn in enumerate(turns):
            if turn == "":
                break
            messages = turn.split(roles[1])
            if len(messages) != 2:
                continue
            for message in messages:
                if message.startswith(roles[0]):
                    #print("roles[0]: ###%s###, message: ###%s###" %  (roles[0], message + roles[1]))
                    ...
                    label = [IGNORE_INDEX] * len(tokenize_text)
                else:
                    #print("roles[1]: ###%s###, message: ###%s###" %  (roles[1], message))
                    tokenize_text = tokenizer(
                        message,
                        padding=False,
                        max_length=max_seq_length,
                ...
                # remove bos and eos id from seperate token ids
            ...
            # add seperator when each round of dialogue ends
            if i < len(turns) - 1:
                sep_tokens = tokenizer(seperators[1])["input_ids"][1:-1]
                # loss calc end at </s>
                tokenized_text.extend(sep_tokens)
                sep_tokens[-1] = IGNORE_INDEX
                labeled.extend(sep_tokens)
...
def main():
    ...
    if model_args.model_name_or_path:
        ...
        # lora training, scale weight by alpha/r, alpha is rounghly the same as lr
